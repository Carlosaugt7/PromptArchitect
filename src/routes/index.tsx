...
import {
  ...
  PromptArtifactPanel,
  looksLikePrompt,
} from "@/components/PromptArtifact";
...
// SEO Meta Tags: name="description" property="og:title"
  ...
  head: () => ({
    meta: [
      { title: "PromptArchitect — Crie prompts com IA" },
      {
        name: "description",
        content:
          "Crie, refine e otimize prompts para LLMs com agentes especializados em prompt engineering.",
      },
    ],
  }),
...
function PromptArchitect() {
  ...
  const [agents, setAgents] = useState<AgentsState>({
    leadId: "prompt-architect",
    activeIds: ["prompt-architect"],
  });
    ...
    function onKey(e: KeyboardEvent) {
      ...
      if (e.key === "ArrowUp" && document.activeElement === textareaRef.current && input === "") {
        ...
        if (last) {
          ...
          setEditingMsgText(typeof last.content === "string" ? last.content : "");
        }
      }
    }
  ...
  async function runTurn(
    ...
    if (!sel) {
      toast.error("Selecione um modelo nas configurações");
      return;
    }
    ...
    try {
      ...
      const { usage } = await runOrchestration(
        ...
        userContent,
        agents.activeIds,
        agents.leadId ?? "prompt-architect",
  ...
  async function handleImport(list: FileList | null) {
    ...
    for (const f of Array.from(list)) {
      try {
        importConversation(parseImportedConversation(await f.text(), f.name));
        count++;
      } catch (e) {
        ...
        <div className="flex flex-col h-full w-72 min-w-0">
          ...
          <div className="flex items-center justify-between px-4 py-3.5 border-b border-border/40 shrink-0">
            <div className="flex items-center gap-2.5">
              <Logo />
              <span className="font-display font-semibold text-sm gradient-text whitespace-nowrap">
                PromptArchitect
              </span>
            </div>
            <button
                ...
                {conversation.messages.length > 0 && (
                  <>
                    <button
                      ...
                      disabled={sending}
                      title="Regenerar"
                      className="grid h-8 w-8 place-items-center rounded-lg text-muted-foreground hover:bg-accent disabled:opacity-40 transition-colors"
                    >
                    ...
                    {conversation.messages.map((m) => (
                      <MsgBubble
                        ...
                        onEditStart={() => {
                          ...
                          setEditingMsgText(typeof m.content === "string" ? m.content : "");
                        }}
                        onEditSave={saveEdit}
            ...
            <div className="border-t border-border/30 bg-background/80 backdrop-blur-sm px-4 py-3 shrink-0">
              <div className="max-w-3xl mx-auto">
                ...
                {attachments.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {attachments.map((a) => (
                      <span
                        key={a.id}
                        className="flex items-center gap-1.5 rounded-lg border border-border bg-card/60 px-2 py-1 text-[11px]"
                      >
                        {a.kind === "image" ? (
                          <img
                            src={a.content}
                          ...
                          <FileType2 className="h-3.5 w-3.5 text-red-400" />
                        ) : (
                ...
                <div className="relative rounded-2xl border border-border bg-card/60 focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/20 transition-all shadow-sm">
                  ...
                  <div className="flex items-center justify-between px-3 pb-3 pt-1 gap-2">
                    ...
                    {sending ? (
                      ...
                      <div
                        className="grid h-9 w-9 place-items-center rounded-xl bg-primary/20 border border-primary/30 shrink-0"
                        title="Buscando na web…"
                      >
                        <Globe
                ...
                <p className="mt-2 text-center text-[11px] text-muted-foreground/50">
                  Ctrl+Enter envia · Ctrl+K nova conversa · Esc para stream · ↑ edita última
                </p>
              </div>
            </div>
          </>
...
function SidebarFooterBtn({
  ...
  return (
    <button
      onClick={onClick}
      className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
    >
...
function SidebarGroup({
  ...
  return (
    <div className="pt-2">
      <p className="px-2 pb-1 text-[10px] uppercase tracking-wider font-medium text-muted-foreground/50">
        {label}
      </p>
      {items.map((c) => (
        <SidebarConvoItem key={c.id} c={c} active={c.id === activeId} onOpen={() => onOpen(c)} />
      ))}
    </div>
  );
}

function SidebarConvoItem({
  c,
  active,
  onOpen,
}: {
  ...
  return (
    <div
      className={`group flex items-center gap-2 rounded-lg px-2 py-1.5 cursor-pointer transition-colors ${active ? "bg-accent/70 text-foreground" : "text-muted-foreground hover:bg-accent/40 hover:text-foreground"}`}
          ...
          {editing ? (
            <div className="space-y-2">
              <textarea
                ...
                className="w-full bg-transparent text-sm resize-none focus:outline-none min-w-[280px]"
                rows={3}
                autoFocus
              />
              <div className="flex gap-2 justify-end">
                ...
                <button
                  onClick={onEditSave}
                  className="px-3 py-1 text-xs rounded-lg bg-primary text-primary-foreground hover:opacity-90 transition-colors"
                >
                  Salvar & Reenviar
                </button>
              </div>
...
const SUGGESTIONS = [
  {
    ...
    label: "Criar prompt do zero",
    text: "/criar Preciso de um prompt para um assistente de suporte ao cliente que responda em PT-BR, seja empático e direcione escaladas corretamente.",
  },
  {
    icon: <BookOpen className="h-5 w-5" />,
    label: "Otimizar prompt existente",
    text: "/otimizar ",
  },
  {
    icon: <Code2 className="h-5 w-5" />,
    label: "Prompt para código",
    text: "/codigo Crie um prompt para um assistente que revisa código TypeScript seguindo as melhores práticas de clean code e segurança.",
  },
  {
    icon: <ScrollText className="h-5 w-5" />,
    label: "PRD completo",
    text: "/prd Preciso de um PRD para um sistema de chat com suporte a múltiplos modelos de IA, histórico de conversas e exportação.",
  },
  {
    icon: <Zap className="h-5 w-5" />,
    label: "Chain of Thought",
    text: "Crie um prompt que instrua o modelo a raciocinar passo a passo antes de dar a resposta final, usando a técnica Chain of Thought para problemas matemáticos.",
  },
  {
    icon: <Bot className="h-5 w-5" />,
    label: "Persona de agente",
    text: "Defina a persona e as regras de comportamento para um agente especializado em análise de dados que comunica insights de forma clara para não-técnicos.",
  },
  {
    icon: <FileText className="h-5 w-5" />,
    label: "Termo de Referência",
    text: "/documento Termo de Referência para contratação de serviço de desenvolvimento de software sob medida, modalidade pregão eletrônico, valor estimado R$ 180.000,00, prazo 12 meses.",
  },
  {
    icon: <ScrollText className="h-5 w-5" />,
    label: "Memorando executivo",
    text: "/memorando Para: Diretoria de TI | Assunto: Solicitação de aprovação de orçamento para renovação de licenças de software | Contexto: licenças vencem em 30 dias, impacto operacional crítico.",
  },
];
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm max-w-md mx-auto leading-relaxed">
          ...
          {lead && (
            <span>
              {" "}
              Usando <strong className="text-foreground">{lead.name}</strong> como agente principal.
            </span>
          )}
        </p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 w-full max-w-2xl">
        {SUGGESTIONS.map((s, i) => (
          ...
          >
            <span className="text-primary/70 group-hover:text-primary transition-colors mt-0.5 shrink-0">
              {s.icon}
            </span>
      ...
      <p className="mt-6 text-xs text-muted-foreground/50">
        Digite <kbd className="bg-card border border-border rounded px-1 py-0.5 text-[10px]">/</kbd>{" "}
