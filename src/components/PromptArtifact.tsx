...
import {
  FileText, Download, X, Copy, Check, ChevronRight,
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  h1 { font-size: 22px; font-weight: 700; color: #5b21b6; margin: 24px 0 10px; border-bottom: 2px solid #ede9fe; padding-bottom: 6px; }
  h2 { font-size: 17px; font-weight: 600; color: #4c1d95; margin: 20px 0 8px; }
  h3 { font-size: 15px; font-weight: 600; color: #6d28d9; margin: 16px 0 6px; }
  p { margin: 8px 0; }
  ul { margin: 8px 0 8px 20px; }
  li { margin: 3px 0; }
  code { font-family: 'JetBrains Mono', monospace; font-size: 12px;
         background: #f5f3ff; color: #5b21b6; padding: 1px 5px; border-radius: 4px; }
  ...
  .header { display: flex; align-items: center; gap: 12px; margin-bottom: 32px;
            padding-bottom: 20px; border-bottom: 1px solid #ede9fe; }
  .logo { width: 36px; height: 36px; background: linear-gradient(135deg, #7c3aed, #a855f7);
          border-radius: 8px; display: flex; align-items: center; justify-content: center;
          color: white; font-weight: 700; font-size: 16px; }
  .brand { font-weight: 700; font-size: 15px; color: #5b21b6; }
...
export function PromptArtifactCard({ content, onOpen }: CardProps) {
  ...
  return (
    <div className="mt-4 flex items-center gap-3 rounded-xl border border-primary/20 bg-primary/5 hover:bg-primary/8 px-4 py-3 transition-colors group">
      {/* Ícone documento */}
      ...
      >
        ...
        <div className="border-t border-border/40 px-4 py-3 shrink-0 flex items-center justify-between">
          <p className="text-[11px] text-muted-foreground">
            {content.length.toLocaleString("pt-BR")} caracteres · {content.split(/\s+/).filter(Boolean).length.toLocaleString("pt-BR")} palavras
          </p>
