...
export function CompareDialog({
  ...
  useEffect(() => {
    if (open) {
      setAvailable(listEnabledModels());
      setInput(prompt);
    }
  }, [open, prompt]);
  ...
  function toggle(key: string) {
    setPicked((p) =>
      p.includes(key) ? p.filter((k) => k !== key) : p.length < 3 ? [...p, key] : p,
    );
  }

  async function run() {
    if (!input.trim() || picked.length < 2) {
      toast.error("Escolha 2–3 modelos e escreva um prompt");
      return;
    }
    ...
    await Promise.all(
      newRuns.map(async (r) => {
        try {
          const { usage } = await sendChatStream(
            r.sel,
            messages,
            (chunk) => {
              r.text += chunk;
    ...
    >
      <DialogContent className="max-w-5xl w-[95vw] max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Envie o mesmo prompt para 2–3 modelos e veja as respostas lado a lado.
          </DialogDescription>
        </DialogHeader>

        <div className="flex flex-wrap gap-1.5">
          {available.length === 0 && (
            <p className="text-sm text-muted-foreground">Nenhum modelo habilitado.</p>
          )}
          {available.map((m) => {
        ...
        <div className="flex items-center justify-end gap-2">
          <button
            onClick={stopAll}
            className="rounded-lg border border-border bg-card/60 px-3 py-1.5 text-sm hover:bg-accent"
          >
            <X className="h-3.5 w-3.5 inline mr-1" /> Parar
          </button>
          <button
            onClick={run}
            className="rounded-lg bg-primary text-primary-foreground px-3 py-1.5 text-sm hover:opacity-90"
          >
        ...
        <div
          className="grid gap-3 overflow-auto flex-1"
          style={{ gridTemplateColumns: `repeat(${Math.max(runs.length, 1)}, minmax(0, 1fr))` }}
        >
          {runs.map((r, i) => (
            <div
              key={i}
              className="flex flex-col rounded-lg border border-border bg-card/40 p-3 min-h-[200px]"
            >
